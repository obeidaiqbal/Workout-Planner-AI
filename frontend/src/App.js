...
export default function BioDataForm() {
  ...
  const [mode, setMode] = useState('imperial'); // metric or imperial
  ...
  const handleSubmit = (e) => {
    ...
    .then((res) => res.json())
    .then((json) => console.log('FastAPI response:', json))
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <select className="form-select" name="frequency" value={data.frequency} onChange={handleChange} required>
            ...
            <option value="34">3-4 days</option>
            <option value="45">4-5 days</option>
            <option value="56">5-6 days</option>
          </select>
        </div>
